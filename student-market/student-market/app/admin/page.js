...
export default function StudentsPage() {
  ...
  async function handleCsvUpload(e) {
    ...
    // Strip a UTF-8 BOM if Excel added one at the start of the file.
    const cleanText = text.replace(/^\uFEFF/, "");
    const lines = cleanText.split(/\r?\n/).filter((l) => l.trim());

    // Auto-detect the delimiter: Excel (especially Dutch/European locales)
    // often exports semicolon-separated files instead of comma-separated.
    const delimiter = (lines[0] || "").includes(";") ? ";" : ",";

    // Supports optional header row containing "number"/"name".
    let dataLines = lines;
    ...
    const rows = dataLines
      ...
      .map((parts) => ({
        ...
        name: parts.slice(1).join(delimiter).trim(),
      }))
